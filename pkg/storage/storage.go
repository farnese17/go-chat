...
func (ls *LocalStorage) Upload(file multipart.File, filename string) (string, error) {
    ...
    if err != nil {
        return "", errorsx.ErrUploadFailed
    }
    ...
    if err := ls.DB.IsExist(name, dirPath); err != nil && !errors.Is(err, ErrNotFound) {
        if errors.Is(err, ErrFileExisted) {
            time := time.Now().UnixMilli()
            name += "_" + strconv.FormatInt(time, 10)
        } else {
            return "", err
        }
    }
    ...
    if err := ls.DB.SaveFilePath(name, dirPath); err != nil {
        return "", errorsx.ErrUploadFailed
    }
...
func (ls *LocalStorage) Download(name string) (string, error) {
    return ls.DB.GetFilePath(name)
}

func (ls *LocalStorage) Delete(name string) error {
    return ls.DB.DeleteFile(name)
}
...
// // 获取文件后缀
// func (ls *LocalStorage) GetFileType(filename string) string {
//  idx := strings.LastIndex(filename, ".")
//  if idx == -1 || len(filename[idx:]) > 5 || len(filename[idx:]) < 2 {
//      return "unkown/"
//  }
