    ...
    for i, tt := range tests {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("create group %s", tt.Name), func(t *testing.T) {
                ...
                req := httptest.NewRequest("POST", "/api/v1/group/create", bytes.NewBuffer(body))
                var uid uint
                if i < len(testData) {
                    uid = testData[i].ID
                }
    ...
    for range testDataCount {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("search group by gid %d", tt.GID), func(t *testing.T) {
                url := fmt.Sprintf("/api/v1/group/search/gid/%d", tt.GID)
    ...
    for i := range testDataCount {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("search group by name %d", i), func(t *testing.T) {
                url := fmt.Sprintf("/api/v1/group/search/name/%s", "group")
                body, _ := json.Marshal(&m.Cursor{PageSize: 1, HasMore: true})
                req := httptest.NewRequest("GET", url, bytes.NewBuffer(body))
                ...
                assert.Equal(t, 1, int(cursor["pagesize"].(float64)))
                assert.Equal(t, true, cursor["hasmore"].(bool))
                assert.Equal(t, uint(1e9)+1, uint(cursor["lastid"].(float64)))
                var g []*m.Group
    ...
    for _, tt := range tests {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("use cursor search group by name %v", tt.cursor), func(t *testing.T) {
                ...
                resp := equalHttpResp(t, w)
                cursor := resp["data"].(map[string]any)["cursor"].(map[string]any)
                assert.Equal(t, tt.expectedCursor.PageSize, int(cursor["pagesize"].(float64)))
                assert.Equal(t, tt.expectedCursor.HasMore, cursor["hasmore"].(bool))
                assert.Equal(t, tt.expectedCursor.LastID, uint(cursor["lastid"].(float64)))
                var g []*m.Group
                jsong, _ := json.Marshal(resp["data"].(map[string]any)["groups"])
                json.Unmarshal(jsong, &g)
                for i := range g {
...
func TestInvite(t *testing.T) {
    ...
    from := strconv.FormatUint(uint64(testData[0].ID), 10)
    gid := strconv.FormatUint(uint64(testGroupData[0].GID), 10)
    for range testDataCount {
        wg.Add(1)
        go func() {
            defer wg.Done()
            ...
            t.Run(fmt.Sprintf("invite user join group %d", tt.ID), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/invite", nil)
                addToken(testData[0].ID, req)
                q := req.URL.Query()
                q.Add("from", from)
                q.Add("to", strconv.FormatUint(uint64(tt.ID), 10))
                q.Add("gid", gid)
...
func TestAcceptInvite(t *testing.T) {
    ...
    for i := range testDataCount {
        ...
        go func(msg ws.ChatMsg) {
            defer wg.Done()
            t.Run(fmt.Sprintf("accept invite %d", msg.From), func(t *testing.T) {
                body, _ := json.Marshal(msg)
                req := httptest.NewRequest("POST", "/api/v1/group/invite/accept", bytes.NewBuffer(body))
                addToken(msg.To, req)
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
                resp := equalHttpResp(t, w)
    ...
    for i := range testDataCount {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("%d apply join group %d", uid, gid), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/apply", nil)
                addToken(uid, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(gid), 10))
                req.URL.RawQuery = q.Encode()
    ...
    for _, tt := range testGroupData[1:] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("accept join group %d", tt.GID), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/apply/accept", nil)
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                q.Add("to", strconv.FormatInt(int64(testGroupData[0].Owner), 10))
                req.URL.RawQuery = q.Encode()
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
    ...
    for _, tt := range testGroupData[1:] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("reject join group %d", tt.GID), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/apply/reject", nil)
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("to", strconv.FormatUint(uint64(testGroupData[0].Owner), 10))
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                req.URL.RawQuery = q.Encode()
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
    ...
    for range testDataCount {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("get member %d", tt.GID), func(t *testing.T) {
                req := httptest.NewRequest("GET", "/api/v1/group/member", nil)
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("id", strconv.FormatUint(uint64(tt.Owner), 10))
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                req.URL.RawQuery = q.Encode()
    ...
    for range testDataCount {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("get group members %d", tt.GID), func(t *testing.T) {
                url := fmt.Sprintf("/api/v1/group/members/%d", tt.GID)
                req := httptest.NewRequest("GET", url, nil)
    ...
    for _, tt := range testGroupData {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("delete group %d", tt.GID), func(t *testing.T) {
                url := fmt.Sprintf("/api/v1/group/delete/%d", tt.GID)
...
func TestUpdateGroupInformation(t *testing.T) {
    ...
    subTest := func(column, value string) {
        ...
        t.Run(fmt.Sprintf("update group information %d", tt.GID), func(t *testing.T) {
            req := httptest.NewRequest("POST", "/api/v1/group/update", nil)
            addToken(tt.Owner, req)
            q := req.URL.Query()
            q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
            q.Add("field", column)
    ...
    for _, tt := range testGroupData[1:] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("hand over owner %d", tt.GID), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/handoverowner", nil)
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                q.Add("id", strconv.FormatUint(uint64(testGroupData[0].Owner), 10))
                req.URL.RawQuery = q.Encode()
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
...
func TestModifyAdmin(t *testing.T) {
    ...
    subTest := func(tt *m.Group, status int) {
        ...
        t.Run(fmt.Sprintf("modify admin %d", tt.GID), func(t *testing.T) {
            req := httptest.NewRequest("POST", "/api/v1/group/modifyadmin", nil)
            addToken(tt.Owner, req)
            q := req.URL.Query()
            q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
            q.Add("id", strconv.FormatUint(uint64(testGroupData[0].Owner), 10))
    ...
    for _, tt := range testGroupData[1:] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("admin resign%d", tt.GID), func(t *testing.T) {
                url := fmt.Sprintf("/api/v1/group/adminresign/%d", tt.GID)
                req := httptest.NewRequest("POST", url, nil)
                addToken(testData[0].ID, req)
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
    ...
    for _, tt := range testGroupData[1:] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("leave group %d", tt.GID), func(t *testing.T) {
                url := fmt.Sprintf("/api/v1/group/leave/%d", tt.GID)
                req := httptest.NewRequest("POST", url, nil)
                addToken(testGroupData[0].Owner, req)
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
    ...
    for _, tt := range testGroupData[1:] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("kick member %d", tt.GID), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/kick", nil)
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                q.Add("to", strconv.FormatUint(uint64(testGroupData[0].Owner), 10))
    ...
    for _, tt := range testGroupData {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("craete announce %d", tt.GID), func(t *testing.T) {
                ...
                req := httptest.NewRequest("POST", "/api/v1/group/announce/release", bytes.NewBuffer(body))
                addToken(tt.Owner, req)
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
                resp := equalHttpResp(t, w)
    ...
    for _, tt := range testGroupData {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("view announce %d", tt.GID), func(t *testing.T) {
                ...
                req := httptest.NewRequest("GET", "/api/v1/group/announce/view", bytes.NewBuffer(body))
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                req.URL.RawQuery = q.Encode()
                ...
                resp := equalHttpResp(t, w)
                cursor := resp["data"].(map[string]any)["cursor"].(map[string]any)
                assert.Equal(t, float64(15), cursor["pagesize"].(float64))
                assert.Equal(t, float64(math.MaxUint64), cursor["lastid"].(float64))
                assert.Equal(t, false, cursor["hasmore"].(bool))
    ...
    for range testDataCount {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("view latest announce %d", tt.GID), func(t *testing.T) {
                req := httptest.NewRequest("GET", "/api/v1/group/announce/viewlatest", nil)
                addToken(tt.Owner, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(tt.GID), 10))
                req.URL.RawQuery = q.Encode()
    ...
    for _, tt := range testAnnounceData {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("delete announce %d", tt.GroupID), func(t *testing.T) {
                req := httptest.NewRequest("POST", "/api/v1/group/announce/delete", nil)
                addToken(tt.CreatedBy, req)
                q := req.URL.Query()
                q.Add("gid", strconv.FormatUint(uint64(tt.GroupID), 10))
                q.Add("id", strconv.FormatUint(uint64(tt.ID), 10))
                req.URL.RawQuery = q.Encode()
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
                resp := equalHttpResp(t, w)
    ...
    for i, tt := range testGroupData[:len(testGroupData)-1] {
        ...
        go func() {
            ...
            t.Run(fmt.Sprintf("get group list %d", tt.Owner), func(t *testing.T) {
                req := httptest.NewRequest("GET", "/api/v1/group/list", nil)
                addToken(tt.Owner, req)
                w := httptest.NewRecorder()
                route.ServeHTTP(w, req)
